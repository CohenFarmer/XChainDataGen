...
class CctpHandler(BaseHandler):
    ...
    def bind_db_to_repos(self):
        ...
        self.blockchain_transaction_repo = CCTPBlockchainTransactionRepository(
            DBSession
        )
    ...
    def handle_events(
        ...
        for event in events:
            try:
                if event["topic"]   == "0x2fa9ca894982930190727e75500a97d8dc500233a5065e0f3126c48fbe0343c0": # DepositForBurn
                    event = self.handle_deposit_for_burn(blockchain, event)
                elif event["topic"] == "0x58200b4c34ae05ee816d710053fff3fb75af4395915d3d2a771b24aa10e3cc5d": # MessageReceived
    ...
    def handle_deposit_for_burn(self, blockchain, event):
        ...
        destination_chain = self.convert_doman_id_to_blockchain_name(
        ...
        try:
            if self.cctp_deposit_for_burn_repo.event_exists(event["nonce"], blockchain, destination_chain):
                return None
    ...
    def handle_message_received(self, blockchain, event):
        ...
        src_blockchain = self.convert_doman_id_to_blockchain_name(
            event["sourceDomain"]
        )
        ...
        if len(event["messageBody"]) != 264: # there are other messages with different length that we don't want to process
            return None

        message_body = MessageBodyDecoder.decode(event["messageBody"])

        try:
            if self.cctp_message_received_repo.event_exists(event["nonce"], src_blockchain, blockchain):
                return None
