...
class RoninHandler(BaseHandler):
    ...
    def bind_db_to_repos(self):
        ...
        self.blockchain_transaction_repo = RoninBlockchainTransactionRepository(
            DBSession
        )
    ...
    def handle_events(
        ...
        for event in events:
            try:
                if event["topic"] == "0xd7b25068d9dc8d00765254cfb7f5070f98d263c8d68931d937c7362fa738048b":  # DepositRequested
                    event = self.handle_deposit_requested(blockchain, event)
                elif event["topic"] == "0x8d20d8121a34dded9035ff5b43e901c142824f7a22126392992c353c37890524":  # Deposited
                    event = self.handle_tokens_deposited(blockchain, event)
                elif event["topic"] == "0xf313c253a5be72c29d0deb2c8768a9543744ac03d6b3cafd50cc976f1c2632fc":  # WithdrawalRequested
                    event = self.handle_withdrawal_requested(blockchain, event)
                elif event["topic"] == "0x21e88e956aa3e086f6388e899965cef814688f99ad8bb29b08d396571016372d":  # Withdrew
    ...
    def handle_deposit_requested(self, blockchain, event):
        ...
        try:
            [ receipt, mainchain_data, ronin_data, token_info ] = self.extract_objects_from_event(event)

            if self.deposit_requested_repo.event_exists(
                receipt["id"]
            ):
                return None
    ...
    def handle_tokens_deposited(self, blockchain, event):
        ...
        try:
            ...
            if self.token_deposited_repo.event_exists(
                receipt["id"]
            ):
                return None
    ...
    def handle_withdrawal_requested(self, blockchain, event):
        ...
        try:
            ...
            if self.withdrawal_requested_repo.event_exists(
                receipt["id"]
            ):
                return None
    ...
    def handle_token_withdrew(self, blockchain, event):
        ...
        try:
            ...
            if self.token_withdrew_repo.event_exists(
                receipt["id"]
            ):
                return None
