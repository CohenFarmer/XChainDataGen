...
class AcrossGenerator(BaseGenerator):
    ...
    def bind_db_to_repos(self):
        ...
        self.across_cross_chain_token_transfers_repo = AcrossCrossChainTransactionRepository(DBSession)
    ...
    def fix_token_symbol_clashes(self):
        """
        The calculate_cctx_usd_values function calculated the USD value of the input and output amounts, based on the symbol.
        The reason why we base it on the symbol only (and not symbol + name) is because in multiple blockchains, the same token can have different names.
        Therefore, asking for the price of the same token in multiple blockchains would return different prices (e.g., 500 token X worth of 1 USD in one blockchain,
        and 500 token X worth of 2 USD in another blockchain, leading to fake token price disparities).
        However, this can cause problems when multiple tokens have the same symbol. This funcion, serves to fix those problems.
        Unfortunately, the only way to fix this is by manually checking the generated cctxs, checking values that are completely disparate, retrieving token contracts
        and updating the query to fetch the correct token price.

        An example is PEPE and BasedPEPE. Both have the same symbol, but different names. The price of PEPE at the moment is X USD, and the price of BasedPEPE is X*10^-3 USD.
        """
        ...
        try:
            ...
            raise CustomException(
                ...
                func_name,
                f"Error processing USD values for BasedPEPE in across_cross_chain_transactions. Error: {e}",
            )
