...
class RouterGenerator(BaseGenerator):
    ...
    def bind_db_to_repos(self):
        self.router_blockchain_tx_repo = RouterBlockchainTransactionRepository(DBSession)
        ...
        self.cctx_repo = RouterCrossChainTransactionRepository(DBSession)
    ...
    def generate_cross_chain_data(self):
        ...
        try:
            ...
            start_ts = int(self.router_blockchain_tx_repo.get_min_timestamp()) - 86400
            end_ts = int(self.router_blockchain_tx_repo.get_max_timestamp()) + 86400
            ...
            exception = CustomException(
                self.CLASS_NAME,
                func_name,
                f"Error generating Router cross chain transactions: {e}",
            )
    ...
    def match_token_transfers(self):
        ...
        query = text(
            ...
            SELECT
                ...
                dep.deposit_id as deposit_id,
                dep.depositor as depositor,
                dep.recipient_raw as recipient,
                dep.src_token as src_contract_address,
                dep.dest_token_raw as dst_contract_address,
                dep.amount as input_amount,
                NULL as input_amount_usd,
                dep.dest_amount as output_amount,
                NULL as output_amount_usd,
                dep.message_hash as message_hash
            ...
            JOIN router_funds_paid fp ON fp.message_hash = dep.message_hash
            JOIN router_blockchain_transactions dst_tx ON dst_tx.transaction_hash = fp.transaction_hash
