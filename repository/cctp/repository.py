...
class CCTPDepositForBurnRepository(BaseRepository):
    ...
    def event_exists(self, nonce: str, blockchain: str, dst_blockchain: str):
        with self.get_session() as session:
            return session.query(CCTPDepositForBurn).filter(
...
class CCTPMessageReceivedRepository(BaseRepository):
    ...
    def event_exists(self, nonce: int, src_blockchain: str, blockchain: str):
        with self.get_session() as session:
            return session.query(CCTPMessageReceived).filter(
...
class CctpCrossChainTransactionsRepository(BaseRepository):
    ...
    def get_by_src_tx_hash(self, src_tx_hash: str):
        with self.get_session() as session:
            return session.query(CctpCrossChainTransactions).filter(
                CctpCrossChainTransactions.src_transaction_hash == src_tx_hash
            ).first()
...
Index("cctp_deposit_for_burn_nonce_blockchain_idx", CCTPDepositForBurn.nonce, CCTPDepositForBurn.dst_blockchain, CCTPDepositForBurn.blockchain)
...
Index("cctp_message_received_nonce_blockchain_idx", CCTPMessageReceived.nonce, CCTPMessageReceived.blockchain, CCTPMessageReceived.src_blockchain)
