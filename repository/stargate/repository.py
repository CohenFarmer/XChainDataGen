...
class StargateOFTSentRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateOFTSent).filter(StargateOFTSent.transaction_hash == transaction_hash).first()

    def event_exists(self, transaction_hash: str, guid: str, amount_received_ld: int):
        with self.get_session() as session:
            return session.query(StargateOFTSent).filter(
...
class StargateOFTReceivedRepository(BaseRepository):
    ...
    def get_by_guid(self, guid: str):
        with self.get_session() as session:
            return session.query(StargateOFTReceived).filter(StargateOFTReceived.guid == guid).first()

    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateOFTReceived).filter(StargateOFTReceived.transaction_hash == transaction_hash).first()
...
class StargateOFTSendToChainRepository(BaseRepository):
    ...
    def event_exists(self, transaction_hash: str, dst_blockchain: str):
        with self.get_session() as session:
            return session.query(StargateOFTSendToChain).filter(
...
class StargateOFTReceiveFromChainRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateOFTReceiveFromChain).filter(StargateOFTReceiveFromChain.transaction_hash == transaction_hash).first()

    def event_exists(self, transaction_hash: str, src_blockchain: str):
        with self.get_session() as session:
            return session.query(StargateOFTReceiveFromChain).filter(
...
class StargateUlnConfigSetRepository(BaseRepository):
    ...
    def event_exists(self, transaction_hash: str, dst_blockchain: str, oapp: str):
        with self.get_session() as session:
            return session.query(StargateUlnConfigSet).filter(
...
class StargateExecutorFeePaidRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateExecutorFeePaid).filter(StargateExecutorFeePaid.transaction_hash == transaction_hash).first()
...
class StargateDVNFeePaidRepository(BaseRepository):
    ...
    def get_total_fee_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(func.sum(StargateDVNFeePaid.fee)).filter(StargateDVNFeePaid.transaction_hash == transaction_hash).scalar()
...
class StargateBusRodeRepository(BaseRepository):
    ...
    def get_by_ticket_id(self, ticket_id: str):
        with self.get_session() as session:
            return session.query(StargateBusRode).filter(StargateBusRode.ticket_id == ticket_id).first()

    def event_exists(self, transaction_hash: str, ticket_id: str):
        with self.get_session() as session:
            return session.query(StargateBusRode).filter(
...
class StargateBusDrivenRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateBusDriven).filter(StargateBusDriven.transaction_hash == transaction_hash).first()
...
class StargatePacketReceivedRepository(BaseRepository):
    ...
    def get_by_nonce(self, nonce: int):
        with self.get_session() as session:
            return session.query(StargatePacketReceived).filter(StargatePacketReceived.nonce == nonce).first()

    def event_exists(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargatePacketReceived).filter(StargatePacketReceived.transaction_hash == transaction_hash).first()
...
class StargatePacketRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargatePacket).filter(StargatePacket.transaction_hash == transaction_hash).first()

    def event_exists(self, transaction_hash: str, dst_blockchain: str, nonce: int):
        with self.get_session() as session:
            return session.query(StargatePacket).filter(
...
class StargatePacketDeliveredRepository(BaseRepository):
    ...
    def get_by_nonce(self, nonce: int):
        with self.get_session() as session:
            return session.query(StargatePacketDelivered).filter(StargatePacketDelivered.nonce == nonce).first()

    def event_exists(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargatePacketDelivered).filter(StargatePacketDelivered.transaction_hash == transaction_hash).first()
...
class StargatePacketVerifiedRepository(BaseRepository):
    ...
    def event_exists(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargatePacketVerified).filter(StargatePacketVerified.transaction_hash == transaction_hash).first()
...
class StargatePayloadVerifiedRepository(BaseRepository):
    ...
    def event_exists(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargatePayloadVerified).filter(StargatePayloadVerified.transaction_hash == transaction_hash).first()
...
class StargateSwapRemoteRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateSwapRemote).filter(StargateSwapRemote.transaction_hash == transaction_hash).first()
...
class StargateComposeSentRepository(BaseRepository):
    ...
    def event_exists(self, guid: str):
        with self.get_session() as session:
            return session.query(StargateComposeSent).filter(StargateComposeSent.guid == guid).first()
...
class StargateComposeDeliveredRepository(BaseRepository):
    ...
    def event_exists(self, guid: str):
        with self.get_session() as session:
            return session.query(StargateComposeDelivered).filter(StargateComposeDelivered.guid == guid).first()
...
class StargateVerifierFeeRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateVerifierFee).filter(StargateVerifierFee.transaction_hash == transaction_hash).first()
...
class StargateRelayerFeeRepository(BaseRepository):
    ...
    def get_by_transaction_hash(self, transaction_hash: str):
        with self.get_session() as session:
            return session.query(StargateRelayerFee).filter(StargateRelayerFee.transaction_hash == transaction_hash).first()
...
class StargateBusCrossChainTransactionRepository(CrossChainRepository):
    ...
    def get_total_amount_usd_transacted(self):
        with self.get_session() as session:
            return session.query(func.sum(StargateBusCrossChainTransaction.amount_received_ld_usd)).scalar()
...
Index("ix_bus_driven_blockchain_tx", StargateBusDriven.blockchain, StargateBusDriven.transaction_hash)
Index("ix_bus_driven_ticket_range", StargateBusDriven.start_ticket_id, StargateBusDriven.num_passengers)
...
Index("ix_oft_received_to_address_blockchain", StargateOFTReceived.to_address, StargateOFTReceived.blockchain)
...
Index("ix_bus_cross_chain_transaction_transaction_hash", StargateBusCrossChainTransaction.user_transaction_hash, StargateBusCrossChainTransaction.dst_transaction_hash)
Index("ix_packet_nonce", StargatePacket.nonce, StargatePacket.transaction_hash)
Index("ix_packet_received_nonce", StargatePacketReceived.nonce, StargatePacketReceived.transaction_hash)


Index("ix_packet", StargatePacket.transaction_hash, StargatePacket.dst_blockchain, StargatePacket.nonce)
...
Index("ix_uln_config_set_transaction_hash", StargateUlnConfigSet.transaction_hash, StargateUlnConfigSet.dst_blockchain, StargateUlnConfigSet.oapp)
Index("ix_oft_sent_transaction_hash", StargateOFTSent.transaction_hash, StargateOFTSent.guid, StargateOFTSent.amount_received_ld)
Index("ix_oft_send_to_chain_transaction_hash", StargateOFTSendToChain.transaction_hash, StargateOFTSendToChain.dst_blockchain)
Index("ix_oft_receive_from_chain_transaction_hash", StargateOFTReceiveFromChain.transaction_hash, StargateOFTReceiveFromChain.src_blockchain)
